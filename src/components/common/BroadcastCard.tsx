...
import type { BroadcastData, Subject } from '../../types/broadcast';
interface Props extends BroadcastData {
...
const BroadcastCard = ({
  ...
  return (
    ...
    >
      <div
        className={clsx(
          'w-16 h-16 rounded-full mb-6 transition-colors flex items-center justify-center',
          bgColor,
          hoverColor
        )}
      >
        <span
          className={clsx(
            'material-symbols-outlined group-hover:text-white transition-colors',
            iconColor
          )}
        >
          {icon}
